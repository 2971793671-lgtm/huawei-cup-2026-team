...
日期：2026-09-24。执行人：B；审核人：A。主运行：`q1-critic-topsis-20260924-r01`。状态：计算与复核完成，待团队审核。本文落实用户选定的 CRITIC 赋权＋TOPSIS 评分模型，不预设团队已经验收。
...
运行命令和输入要求见包根目录 README-delivery.md；完整样本数据位于 data/processed/ 与各 run/artifacts/，不进入公开 Git 包。汇总表、代码、配置、运行记录、图和本报告放入 Git 提交候选包。团队 Reviewer A 的确认尚未完成。
